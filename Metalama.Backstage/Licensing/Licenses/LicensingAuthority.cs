...
{
    private readonly DSA _publicKey = DSA.Create();
    private readonly DSA? _privateKey;

    /// <summary>
    /// Gets the id of the key stored in <see cref="LicenseKeyDataBuilder.SignatureKeyId"/>.
    /// </summary>
    internal byte KeyId { get; }

    private LicensingAuthority( byte keyId, string publicKey, string? privateKey = null )
    {
        this.KeyId = keyId;

        // FromXmlString method is not supported by .NET Core
        this._publicKey.FromXmlString2( publicKey );

        if ( privateKey != null )
        {
            this._privateKey = DSA.Create();
            this._privateKey.FromXmlString2( privateKey, true );
        }
    }

    public static LicensingAuthority GetProductionAuthority( string? privateKey = null )
        => new(
            0,
...
#pragma warning restore CA5350
    ...
    internal bool VerifySignature( byte[] message, byte[] signature ) => this._publicKey.VerifySignature( GetHash( message ), signature );
...
#pragma warning disable CA5351
        return this._privateKey.CreateSignature( GetHash( message ) );
