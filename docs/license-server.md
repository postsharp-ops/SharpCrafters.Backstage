...
> **Rule.** **A build skipped by the version control check never leases either, and reports nothing to the audit.** A host may decide, before it resolves any licence, that the source tree it is compiling is unmodified and that licensing does not apply to it. Nothing downstream then runs: no source is enumerated, no licence is resolved, no seat is taken — and no `ILicense.ReportUse` is called, so the build is absent from the audit. The under-count is deliberate: the audit counts the people who *modify* code, not the people who compile it. It must not be repaired by reporting a use that no licence satisfied, because a `ReportUse` without a `TryConsume` attributes the build to a licence that was never resolved. The doctrine of the check, its accepted limits and the property that turns it on are in [vcs-check.md](vcs-check.md).
