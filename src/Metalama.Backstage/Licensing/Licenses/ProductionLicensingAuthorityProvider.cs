...
/// </summary>
/// <remarks>
/// The keys 0 and 1 are finite field DSA keys and belong to <see cref="DsaLicensingAuthority"/>. They sign every
/// license key issued until 2026 and are kept so that those license keys keep verifying. The key 2 is an Elliptic
/// Curve DSA key and belongs to <see cref="ECDsaLicensingAuthority"/>. It signs the license keys issued afterwards.
/// The identifiers 255 and 254, which the test provider owns, are excluded from this provider.
/// </remarks>
...
{
    /// <summary>
    /// The identifier of the key that signs a production license key.
    /// </summary>
    public const byte SigningKeyId = 2;
    ...
    protected override LicensingAuthority CreateAuthority( byte keyId )
        => keyId switch
        {
            ...
            SigningKeyId => new ECDsaLicensingAuthority( keyId, _keys[keyId] ),
