...
internal sealed class ExceptionReporter : IExceptionReportManager, IExceptionCapturer
{
    ...
    private readonly ExceptionSensitiveDataHelper _scrubber;
    ...
    /// <summary>
    /// Publishes the capture of a report, so that the user interface can show a notification. The report file name
    ...
    /// starts with a known framework, first-party or bundled open-source prefix; any other assembly is user or
    /// third-party code whose name (even a single token), version and file version can identify the user's product
    /// or build, so it is redacted to <c>#user</c>. The prefix list and the boundary rule are those of the namespace
    /// scrubber, so the two never diverge. See #1680.
    /// </summary>
...
#pragma warning restore CA1307
        ...
        IExceptionAdapter? adapter )
    {
        try
        {
            ...
            this._fileSystem.WriteAllText( fileName, this.BuildReport( hash, scenario, classifiedException, adapter, this._scrubber ) );
