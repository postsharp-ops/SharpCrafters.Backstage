...
public sealed class NotifyCommand : BaseCommand<NotifyCommandSettings>
{
    ...
    protected override int Execute( ExtendedCommandContext context, NotifyCommandSettings settings )
    {
        ...
        // The logos of every product family are shipped next to the executable; the profile selects one of them.
        // TODO: serve a different icon in light theme (this one is for the dark theme). The different is only slight.
        var logoName = context.ServiceProvider.GetRequiredBackstageService<ProductProfile>().LogoName;
        var logoPath = Path.Combine( Path.GetDirectoryName( Environment.GetCommandLineArgs()[0] )!, "Resources", $"{logoName}.png" );
