...
/// <summary>
/// A process that matches a <see cref="KillableProcessSpec"/>. It does not own <see cref="Process"/>, which the caller of
/// <see cref="IProcessManager.GetCandidateProcesses"/> disposes.
/// </summary>
internal sealed class KillableProcess
{
    ...
    /// <summary>
    /// Asks the process to shut down, as <c>VBCSCompiler -shutdown</c> does, when its specification allows it.
    /// </summary>
    /// <returns><c>true</c> when the process has exited.</returns>
    ...
    /// Ends the process.
    /// </summary>
    /// <returns><c>true</c> when the process has exited, <c>false</c> when it could not be ended.</returns>
    public bool Kill( out string? errorMessage )
    {
