        ...
        public string GetTempFileName()
        {
            if ( this._serviceProvider == null )
            {
                throw new InvalidOperationException(
                    $"The temporary directory is not known because this {nameof(FileSystem)} was created without a service provider." );
            }

            this._standardDirectories ??= this._serviceProvider.GetRequiredBackstageService<IStandardDirectories>();

            return TempFileUtilities.GetTempFileName( this._standardDirectories.TempDirectory );
        }
