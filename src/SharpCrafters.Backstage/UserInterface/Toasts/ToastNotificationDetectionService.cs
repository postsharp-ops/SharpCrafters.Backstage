    ...
    private void ValidateRegisteredLicense( LicenseRegistrationProperties? license, ref bool notificationReported )
    {
        ...
        switch ( license )
        {
                    ...
                    if ( license.LicenseType == LicenseType.Evaluation )
                    {
                        this._toastNotificationService.Show(
                            new ToastNotification(
                                ...
                                $"Your {this._productName} trial {FormatExpiration( license.ValidTo.Value )}",
                    ...
                    else
                    {
                        this._toastNotificationService.Show(
                            new ToastNotification(
                                ...
                                $"Your {this._productName} license {FormatExpiration( license.ValidTo.Value )}",
                                "Register a new license key to avoid losing functionality." ) );
                    }
            ...
            case { SubscriptionEndDate: not null }
                ...
                if ( this._catalog?.IsFreeLicense( license.Product, license.LicenseType ) == true )
                {
                    // TODO.
                }
                ...
                else
                {
                    this._toastNotificationService.Show(
                        new ToastNotification(
                            ...
                            $"Your {this._productName} subscription {FormatExpiration( license.SubscriptionEndDate.Value )}",
