...
/// <see cref="UserInterfaceInitializationOptions.OpenWelcomePage"/>. The command-line compiler requests it; Visual Studio
...
public sealed class WelcomePageService : IBackstageService
{
    ...
    internal WelcomePageService( IServiceProvider serviceProvider )
    {
        ...
        if ( !serviceProvider.GetRequiredBackstageService<UserInterfaceInitializationOptions>().OpenWelcomePage )
        {
            throw new InvalidOperationException(
                $"{nameof(UserInterfaceInitializationOptions)}.{nameof(UserInterfaceInitializationOptions.OpenWelcomePage)} is false." );
        }
    }
