...
#if METALAMA_BACKSTAGE
        // Resolved untyped so that this constructor does not require the profile to be registered.
        var productProfile = (Application.ProductProfile?) serviceProvider?.GetService( typeof(Application.ProductProfile) );
        this.GlobalLockNamePrefix = productProfile?.GlobalLockNamePrefix ?? DefaultGlobalLockNamePrefix;
#endif
    }
#endif
...
#if METALAMA_BACKSTAGE
    /// <summary>
    /// The prefix of the machine-wide lock names when no <see cref="Application.ProductProfile"/> is registered. It
    /// is the prefix of the product family that existed before the profile was introduced, and the one that the
    /// copies of this file compiled into the Metalama compiler extensions use, so that every process of that product
    /// family opens the same operating system objects.
    /// </summary>
    internal const string DefaultGlobalLockNamePrefix = "Global\\Metalama_";

    /// <inheritdoc />
    public string GlobalLockNamePrefix { get; }
#endif
