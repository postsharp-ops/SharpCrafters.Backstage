...
/// it has resolved the effective <see cref="TelemetryConsent"/> from its <see cref="ITelemetryPolicy"/>. Reporting an
...
{
    /// <summary>
    /// Captures the <paramref name="classifiedException"/> for the given <paramref name="telemetryConsent"/> (already
    /// resolved by the context from its policy; never <see cref="TelemetryConsent.No"/> in practice). When
