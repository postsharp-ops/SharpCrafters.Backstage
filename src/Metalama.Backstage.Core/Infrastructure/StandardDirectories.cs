...
namespace Metalama.Backstage.Infrastructure
{
    ...
    internal sealed class StandardDirectories : IStandardDirectories
    {
        ...
        public StandardDirectories( IServiceProvider serviceProvider )
        {
            ...
            this._directoryName = productProfile.DataDirectoryName;

            var overriddenTempPath = serviceProvider.GetRequiredBackstageService<IEnvironmentVariableProvider>()
                .GetEnvironmentVariable( productProfile.GetEnvironmentVariableName( "TEMP" ) );
            ...
            // The legacy directory name is the name that an earlier version used under the roaming application data
            // directory, and the name used under the user profile directory on the platforms that have no application
            // data directory. It defaults to the current name when the product has no legacy directory.
            var legacyDirectoryName = productProfile.LegacyDataDirectoryName ?? productProfile.DataDirectoryName;

            static string GetApplicationDataDirectory(
                Environment.SpecialFolder applicationDataDirectory,
                string directoryName,
                string userProfileDirectoryName )
            {
                ...
                if ( !string.IsNullOrEmpty( applicationDataParentDirectory ) )
                {
                    ...
                    return Path.GetFullPath( Path.Combine( applicationDataParentDirectory, directoryName ) );
                }
                else
                {
                    ...
                    // The settings go to the home directory of the user, so the hidden legacy name (e.g. ".metalama") is used.
                    // Normalize the path to ensure consistent path separators.
                    return Path.GetFullPath( Path.Combine( applicationDataParentDirectory, userProfileDirectoryName ) );
                }
            }

            // Till Metalama Backstage 2024.1.8, the application data directory was incorrect.
            var incorrectApplicationDataDirectory = GetApplicationDataDirectory(
                Environment.SpecialFolder.ApplicationData,
                legacyDirectoryName,
                legacyDirectoryName );

            var correctApplicationDataDirectory = GetApplicationDataDirectory(
                Environment.SpecialFolder.LocalApplicationData,
                productProfile.DataDirectoryName,
                legacyDirectoryName );
                    ...
                    switch (Directory.Exists( correctApplicationDataDirectory ), Directory.Exists( osxForwardCompatibleApplicationDataDirectory ))
                    {
                        ...
                        case (true, true):
                            ...
                            logger?.Warning?.Log(
                                ...
                                $"This indicates mixing of .NET runtimes before and after .NET 8.0 in older versions of {productProfile.Name}. " +
        ...
        public string TempDirectory
        {
            get
            {
                ...
                {
                    return Path.Combine( this._overriddenTempPath, this._directoryName );
                }

                if ( this._runtimeInformation.IsOSPlatform( OSPlatform.Windows ) )
                {
                    return Path.Combine( Path.GetTempPath(), this._directoryName );
                }
        ...
        public IReadOnlyList<string> LegacyTempDirectories
            => this._overriddenTempPath != null || this._runtimeInformation.IsOSPlatform( OSPlatform.Windows )
                ...
                : new[] { Path.Combine( Path.GetTempPath(), this._directoryName ) };
