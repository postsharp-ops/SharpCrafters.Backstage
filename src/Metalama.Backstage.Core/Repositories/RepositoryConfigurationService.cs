...
{
    /// <summary>
    /// The name of the configuration file at the repository root, from the product profile, or <c>null</c> when the
    /// product has no such file. In the latter case a repository has the default configuration.
    /// </summary>
    ...
    private RepositoryConfigurationResult Resolve( string startingDirectory )
    {
        ...
        {
            // Any repository configuration file located below the repository root is misplaced: it is ignored and reported.
                ...
                if ( this._fileSystem.FileExists( misplacedFile ) )
                {
                    warnings.Add(
                        new RepositoryConfigurationWarning(
                            ...
                            misplacedFile,
                            $"The file '{misplacedFile}' is ignored because '{this._fileName}' must be located at the repository root ('{repositoryRoot}')." ) );
                }
            }

            var rootFile = Path.Combine( repositoryRoot, this._fileName );
