...
{
    /// <summary>
    /// Gets the name of the session cookie in which the token is stored after it has been presented once, for
    /// instance <c>metalama-setup-token</c>.
    /// </summary>
    /// <remarks>
    ...
    /// through the rest of the session. The name depends on the product so that the setup servers of two products
    /// running on the same machine do not read each other's cookie.
    /// </remarks>
    private static string GetCookieName( ProductProfile productProfile ) => $"{productProfile.Name.ToLowerInvariant()}-setup-token";
    ...
    /// <param name="productProfile">The profile of the product, which names the cookie.</param>
    public static void UseSetupWebServerAuthentication( this WebApplication app, string expectedToken, ProductProfile productProfile )
    {
        var expectedTokenBytes = Encoding.UTF8.GetBytes( expectedToken );
        var cookieName = GetCookieName( productProfile );

        app.Use(
            async ( context, next ) =>
            {
                if ( IsTokenValid( context.Request.Cookies[cookieName], expectedTokenBytes ) )
                {
                    await next( context );
